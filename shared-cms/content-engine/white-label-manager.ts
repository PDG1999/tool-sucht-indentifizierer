...
export class WhiteLabelManager {
  private static configCache = new Map<string, WhiteLabelConfig>();
  private static themeCache = new Map<string, ThemeConfig>();
  // private static licenseCache = new Map<string, LicenseConfig>(); // Reserved for future use
  private static basePath = join(__dirname, '..', 'white-label');
  private static themePath = join(__dirname, '..', 'themes');
  ...
  static loadPartnerConfig(partnerId: string): WhiteLabelConfig {
    ...
    try {
      ...
      const config = JSON.parse(configRaw) as WhiteLabelConfig;
  ...
  static loadDefaultConfig(): WhiteLabelConfig {
    ...
    try {
      ...
      return JSON.parse(configRaw) as WhiteLabelConfig;
    } catch (error) {
  ...
  static loadTheme(themeName: string): ThemeConfig {
    ...
    try {
      ...
      const theme = JSON.parse(themeRaw) as ThemeConfig;
  ...
  static createPartnerConfig(partnerId: string, template: 'basic' | 'premium' | 'enterprise'): WhiteLabelConfig {
    ...
    try {
      ...
      const templateConfig = JSON.parse(templateRaw) as WhiteLabelConfig;
  ...
  static getPartnerStats(): { partnerId: string; tools: number; languages: number; lastUpdated: string }[] {
  ...
  private static validateConfig(config: WhiteLabelConfig): void {
    if (!config.partnerId) {
