...
try {
    ...
    writeFileSync(join(partnerOutputDir, 'docker-compose.yml'), `# Generated for ${partnerConfig.partnerName}\n` +
    ...
    for (const tool of partnerConfig.tools) {
        for (const language of partnerConfig.languages) {
            try {
                ...
                writeFileSync(join(contentDir, `${language}.json`), JSON.stringify(content, null, 2));
    ...
    writeFileSync(join(partnerOutputDir, 'deployment-meta.json'), JSON.stringify(deploymentMeta, null, 2));
    ...
    writeFileSync(join(partnerOutputDir, 'coolify.json'), JSON.stringify(coolifyConfig, null, 2));
